...
* @brief Test file for implementing CC1200_transmit()
* @date May 5 2021
*
*/
#include "main.h"
...
HAL_Init();
	...
	char message[50];
	uint8_t packet[] = "I need some sleep!\n";
	char spi_buf[128];
	...
	while (1){
		CC1200_transmit(&xcvr1, packet);

		CC1200_read_register(&xcvr1, CC1200_MARC_STATUS1);
		if(*xcvr1.miso_data == 0x07 || *xcvr1.miso_data == 0x08){
			CC1200_command_strobe(&xcvr1, CC1200_COMMAND_SFTX);
		}

		message_len = sprintf(message, "%x\n",*xcvr1.miso_data);
