...
* @brief Test file for implementing the enchance wake-on-radio function of the cc1200
* @date May 5 2021
*
*/
#include "main.h"
...
// Used to hold the cc1200 GPIO wakeup signal to the STM32 GPIO interrupt
uint8_t mcu_wakeup;
	...
	while (1){
		if(mcu_wakeup == 1){
			...
			if(*(xcvr1.miso_data) == 0x80){
				...
				message_len = sprintf(message, "%s",rx_buf);
				HAL_UART_Transmit(&huart2, (uint8_t*)message, message_len, 100);
			}
